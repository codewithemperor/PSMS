...
// PATCH /api/feedback/[id] — student updates feedback status (PENDING → ADDRESSED | DISMISSED)
...
export async function PATCH(
  ...
  if (
    !newStatus ||
    !["PENDING", "ADDRESSED", "DISMISSED"].includes(newStatus)
  ) {
    return NextResponse.json(
  ...
  const feedback = await db.feedback.findUnique({
    where: { id },
    select: { id: true, studentId: true, authorId: true, content: true },
  })
  if (!feedback) {
    return NextResponse.json(
  ...
  if (session.user.role === "STUDENT") {
    ...
    // Supervisors cannot change feedback status, only students can
    return NextResponse.json(
  ...
  // Notify the feedback author (supervisor) that their feedback was addressed
  if (newStatus === "ADDRESSED") {
    await db.notification.create({
      data: {
        title: "Feedback Addressed",
        message: `Your feedback was marked as addressed by the student.`,
        type: "INFO",
