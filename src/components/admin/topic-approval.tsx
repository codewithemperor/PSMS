...
interface TopicApprovalProps {
  variant?: "admin" | "supervisor"
}

export function TopicApproval({ variant = "admin" }: TopicApprovalProps = {}) {
  ...
  const { data, isLoading } = useQuery<TopicsResponse>({
    queryKey: ["topics", statusFilter, search],
    queryFn: async () => {
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-slate-800">
            {variant === "supervisor" ? "Topic Reviews" : "Topic Approval"}
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            {variant === "supervisor"
              ? "Review and respond to topics submitted by your students"
              : "Review and approve student project topics"}
          </p>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="When students submit topics for review, they will appear here."
        />
      ) : (
            ...
            >
              <Card className="flex h-full flex-col rounded-xl border-slate-200/60 transition-shadow hover:shadow-md">
                <CardContent className="flex flex-1 flex-col p-5">
                  ...
                  {/* Action buttons for PENDING topics — pinned to bottom */}
      ...
      <ConfirmDialog
        ...
        description={`"${approveTarget?.title}" will be marked as approved. A project will be created for ${approveTarget?.studentName} with 5 default milestones, and they will be notified.`}
        ...
        <DialogContent className="max-w-md rounded-xl">
          ...
          <DialogFooter>
            ...
            >
              ...
              {commentTarget?.action === "reject" ? "Reject Topic" : "Send Revision Request"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
